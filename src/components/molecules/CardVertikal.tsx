type CardProps<T> = {
  item: T;
  fields: {
    image: keyof T;
    title: keyof T;
    subtitle?: keyof T;
    description?: keyof T;
  };
  onClick: (item:T) => void;
};

const CardVertikal = <T,>({ item, fields }: CardProps<T>) => {
  return (
    <div className="relative group">
      <figure className="w-full relative">
        <div className="w-full h-full flex items-center justify-center grayscale group-hover:grayscale-0 transition-all duration-300 border">
          <img
            src={String(item[fields.image])}
            alt="image-people"
            className="w-full h-full object-cover"
          />
        </div>

        <div
          className="label-card w-full absolute bottom-5 transform flex flex-col justify-center items-center p-4 bg-[#F8F7F6] text-black rounded
          invisible group-hover:visible group-hover:opacity-100 transition-all duration-300 translate-y-[100px] group-hover:translate-y-0 cursor-pointer"
        >
          <span className="lg:text-xs xl:text-xl font-medium">
            {String(item[fields.title])}
          </span>

          {fields.description && (
            <span className="lg:text-xs xl:text-xl font-medium">
              {String(item[fields.description])}
            </span>
          )}
        </div>
      </figure>
    </div>
  );
};

export default CardVertikal;
